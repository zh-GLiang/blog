...
// 默认配置
const defaultConfig = {
  stepSize: 40, // 每次滚动的像素数
  fps: 0.5, // 帧率
  smoothDuration: 500, // 平滑滚动回顶的持续时间(ms)
  hoverPause: true // 鼠标悬停时暂停
}
export default {
  inserted(el, binding) {
    const config = Object.assign({}, defaultConfig, binding.value)
    const scrollEl = el.querySelector('.el-table__body-wrapper')
    if (!scrollEl) {
      console.warn('AutoScroll directive requires ElementUI el-table component')
      return
    }
    const state = {
      isActive: false,
    ...
    const scrollStep = (timestamp) => {
      ...
      if (!state.lastFrameTime || timestamp - state.lastFrameTime >= interval) {
        ...
        if (state.direction === 'down') {
          ...
          scrollEl.scrollTop += config.stepSize
        }
      }
    ...
    const smoothScrollToTop = () => {
      ...
      state.scrollListener = createScrollEndListener(() => {
        state.direction = 'down'
        scrollStep(performance.now())
      })
      scrollEl.addEventListener('scroll', state.scrollListener)
    }
    const createScrollEndListener = (callback) => {
      let lastScrollTop = scrollEl.scrollTop
      let timeoutId = null
      const threshold = 1
      const checkScrollStopped = () => {
        if (Math.abs(scrollEl.scrollTop - lastScrollTop) < threshold) {
          scrollEl.removeEventListener('scroll', state.scrollListener)
          state.scrollListener = null
          callback()
        } else {
          lastScrollTop = scrollEl.scrollTop
          timeoutId = setTimeout(checkScrollStopped, 50)
        }
      }
      return () => {
        lastScrollTop = scrollEl.scrollTop
    ...
    el._autoScrollConfig = config
    el._autoScrollStart = startAutoScroll
    el._autoScrollStop = stopAutoScroll
  },
  unbind(el) {
    if (el._autoScrollState) {
      el._autoScrollStop()
      if (el._autoScrollState.scrollListener) {
        const scrollEl = el.querySelector('.el-table__body-wrapper')
        if (scrollEl) {
          scrollEl.removeEventListener('scroll', el._autoScrollState.scrollListener)
        }
      }
      if (el._autoScrollConfig.hoverPause) {
        el.removeEventListener('mouseenter', el._autoScrollStop)
        el.removeEventListener('mouseleave', el._autoScrollStart)
      }
    }
  }
}
...
<template>
  <div>
    <el-table :data="tableData" v-auto-scroll> </el-table>
  </div>
</template>
...
```js
function createSafeObject(obj, defaultValue = '-') {
  // 处理非对象输入和null
  if (obj === null || typeof obj !== 'object') {
    return new Proxy({}, { get: () => defaultValue });
  }
  ...
  return new Proxy(obj, {
    get: function (target, prop) {
      ...
      if (typeof value === 'object') {
        return createSafeObject(value, defaultValue);
      }

      // 对于非对象值，返回一个空对象的代理以确保链式访问安全
      return createSafeObject({}, defaultValue);
    }
  });
}
// 测试
...
const safeObject = createSafeObject(testObject);

console.log(safeObject.a.b.d); 
```
...
function nullishCoalesce(value, defaultValue = '--') {
      // 严格判断：仅当 value 是 null 或 undefined 或 空字符串 时，返回默认值
